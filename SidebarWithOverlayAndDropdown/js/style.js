...
$(document).ready(function() {
             ...
             //   return console.log(Number(data[r].basic));
                $("#payslip-container").append(
                    '<div class="col-6 mt-2"><div class="card"><div class="card-header">Period Covered: ' +
                        ...
                        "<hr>" +
                        "<b>Total Earnings:</b>" +
                        "<span>" + ( Number(data[r].basic.replace(',','')) + Number(data[r].overtime.replace(',','')) + Number(data[r].allowance.replace(',','')) ).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + "</span><hr>" +
                        "<b>Deductions</b><br>" +
                        "SSS : " +
                        "<span>" +
                        data[r].sss +
                        ...
                        "<span>" + ( Number(data[r].sss.replace(',','')) + Number(data[r].pag_ibig.replace(',','')) + 
                        Number(data[r].philhealth.replace(',','')) + Number(data[r].sss_loan.replace(',','')) + Number(data[r].pag_ibig_loan.replace(',',''))
                    + Number(data[r].tax.replace(',','')) + Number(data[r].late.replace(',',''))
                    ).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + "</span><hr>" + "</span><hr>" +
                        "<b>Net Pay:</b>" +
                        "<span>" + 
                        ( Number(data[r].basic.replace(',','')) + Number(data[r].overtime.replace(',','')) + Number(data[r].allowance.replace(',','')) + Number(data[r].sss.replace(',','')) + Number(data[r].pag_ibig.replace(',','')) + 
                        Number(data[r].philhealth.replace(',','')) + Number(data[r].sss_loan.replace(',','')) + Number(data[r].pag_ibig_loan.replace(',',''))
                    + Number(data[r].tax.replace(',','')) + Number(data[r].late.replace(',',''))).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")
                        + "</span><hr>" +
                        "</div></div>"
                );
            }
            ...
            for(var d in res) {
                ...
                if((sDate - 1) == datx.getMonth()) {
                    ...
                    $('#timesheetTable').append("<tr>" + 
                        ...
                        "<td colspan='4'>" + res[d].late + "</td>" +
                    +"</tr>");
                  //filter by department  console.log("2");
                }
            }
        });
    })
    ...
    $('#company-filter').on('change',function() {  
        console.log('v'); //when on change 
        httpAjax('get', '/Employee/modules/company/filterJobsByDept', {}).then(res => {
            var c = $('#company-filter').val();
            $('#companyTable').empty();
            for(var allJobs in res) {
                if(c == res[allJobs].dept_name) {
                    $('#companyTable').append("<tr>" + 
                        "<td colspan='5'>" + res[allJobs].title + "</td>" + 
                        "<td colspan='5'>" + res[allJobs].class_name + "</td>" + 
                        "<td colspan='5'>" + res[allJobs].designation + "</td>" + 
                        "<td colspan='5'>" + res[allJobs].dept_name + "</td>" +
                    +"</tr>");
                }else if(c == 'x') {
                    $('#companyTable').append("<tr>" + 
                        "<td colspan='5'>" + res[allJobs].title + "</td>" + 
